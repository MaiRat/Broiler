...
public static class JSValueArrayExtensions
{
    ...
    public static JSValue GetAt(this JSValue[] target, int index, JSValue def = null) => target.Length > index ? target[index] : (def ?? JSUndefined.Value);


    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static JSValue Get1(this JSValue[] target) => target.Length > 0 ? target[0] : JSUndefined.Value;
