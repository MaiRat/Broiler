...
public class JSFunctionBuilder
{
    ...
    public static Expression InvokeSuperConstructor(
        Expression super,
        Expression returnValue, Expression args) => Expression.Assign(returnValue,
            super.CallExpression<JSFunction, JSValue>(() => (x) => x.InvokeSuper(Arguments.Empty), args)
            );//return Expression.Assign(returnValue, //    Expression.Call(null, _invokeSuperConstructor, newTarget, super, args));
