...
public class TypeHelper<T>
{
    ...
    protected static PropertyInfo IndexProperty<T1>() => typeof(T)
            .GetProperties()
    ...
    {
        var a = typeof(T).GetConstructor([typeof(T1)]);
        return a;
    }

    protected static ConstructorInfo Constructor<T1, T2>() => typeof(T).GetConstructor([typeof(T1), typeof(T2)]);
    protected static ConstructorInfo Constructor<T1, T2, T3>()
    {
        var c = typeof(T).GetConstructor([typeof(T1), typeof(T2), typeof(T3)]);
        if (c != null)
            return c;
    ...
    {
        var c = typeof(T).GetConstructor([typeof(T1), typeof(T2), typeof(T3), typeof(T4)]);
        if (c != null)
            return c;
        var list = typeof(T).GetConstructors(BindingFlags.Public | BindingFlags.NonPublic);
        return list.FirstOrDefault(x =>
    ...
    protected static MethodInfo Method(string name) => typeof(T).GetMethod(name);

    protected static MethodInfo Method<T1>(string name)
    {
        var a = typeof(T).GetMethod(name, [typeof(T1)]);
        return a;
    }
    ...
    protected static MethodInfo InternalMethod<T1, T2>(string name)
    {
        var a = typeof(T)
            .GetMethod(name,
                ...
                , null, [typeof(T1), typeof(T2)], null);
        return a;
    }

    protected static MethodInfo Method<T1, T2>(string name) => typeof(T).GetMethod(name, [typeof(T1), typeof(T2)]);

    protected static MethodInfo StaticMethod<T1, T2>(string name) => typeof(T)
            .GetMethod(name,
            [typeof(T1), typeof(T2)]);
    ...
    protected static FieldInfo InternalField(string name) => typeof(T).GetField(name, BindingFlags.NonPublic | BindingFlags.Instance);

    protected static FieldInfo Field(string name) => typeof(T).GetField(name);
