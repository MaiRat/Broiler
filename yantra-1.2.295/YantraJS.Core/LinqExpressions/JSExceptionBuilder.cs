...
public static class JSExceptionBuilder
{
    ...
    public static Expression ThrowSyntaxError(string value) => NewLambdaExpression.StaticCallExpression(
            ...
            Expression.Constant(value));//return Expression.Call(null, _ThrowSyntaxError, Expression.Constant(value));
    ...
    public static Expression Throw(string message,
        ...
        [CallerLineNumber] int line = 0) => Expression.Throw(
            ...
            Expression.Constant(line)), type ?? typeof(JSValue));//return Expression.Throw(Expression.New(_new,//        Expression.Constant(message),//        Expression.Constant(function),//        Expression.Constant(filePath),//        Expression.Constant(line)), type ?? typeof(JSValue));
    ...
    public static Expression Wrap(Expression body) => body;//var b = Expression.Variable(typeof(Exception));//var cb = Expression.Catch(b, //    Expression.Throw(From(b),typeof(JSValue)), //    Expression.Not(Expression.TypeIs(b, typeof(JSException))));//return Expression.Block(new ParameterExpression[] { b }, //    Expression.TryCatch(body, cb )).ToJSValue();
}
