...
public partial class JSUInt16Array : JSTypedArray
{
    ...
    protected internal override JSValue GetValue(uint index, JSValue receiver, bool throwError = true)
    {
        ...
        return new JSNumber((int)BitConverter.ToUInt16(buffer.buffer, byteOffset + (int)index * 2));
    }
    ...
    internal protected override bool SetValue(uint index, JSValue value, JSValue receiver, bool throwError = true)
    {
        if (index < 0 || index >= length)
            return false;
        Array.Copy(BitConverter.GetBytes((UInt16)value.IntValue), 0, buffer.buffer, byteOffset + index * 2, 2);
        return true;
    }

    [JSExport(Length = 1)]
    public static JSValue From(in Arguments a) => new JSUInt16Array(TypedArrayParameters.From(in a, BYTES_PER_ELENENT));
