...
#pragma warning restore 1718
    ...
    public virtual JSValue Negate() => new JSNumber(-DoubleValue);

    public virtual JSValue Subtract(JSValue value) => new JSNumber(DoubleValue - value.DoubleValue);
    ...
    public virtual JSValue BitwiseOr(JSValue value) => new JSNumber(IntValue | value.IntValue);

    public virtual JSValue BitwiseXor(JSValue value) => new JSNumber(IntValue ^ value.IntValue);

    public virtual JSValue LeftShift(JSValue value) => new JSNumber(IntValue << value.IntValue);

    public virtual JSValue RightShift(JSValue value) => new JSNumber(IntValue >> (value.IntValue & 0x1F));

    public virtual JSValue UnsignedRightShift(JSValue value) => new JSNumber(UIntValue >> value.IntValue);

    public virtual JSValue Modulo(JSValue value) => new JSNumber(DoubleValue % value.DoubleValue);
    ...
    protected virtual JSObject GetCurrentPrototype() => null;

    internal abstract PropertyKey ToKey(bool create = true);

    public virtual JSValue GetPrototypeOf() => prototypeChain?.@object ?? JSNull.Value;
    ...
    internal protected virtual bool SetValue(JSSymbol key, JSValue value, JSValue receiver, bool throwError = true) => false;
    ...
    public JSValue Power(JSValue a) {
        ...
        return new JSNumber(Math.Pow(DoubleValue, a1));
    }
